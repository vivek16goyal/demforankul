                        ...
                        {
                            {
                                ...
                                //label: item["0"].PNAME
                                {
                                    localStorage.setItem("label", item[i].PNAME);
                                    item01 = item.length;
                                    Pcode = item[i].PCODE;
                                    label = item[i].PNAME;
                                    Pname = item[i].Add1;
                                    mobno = item[i].ConsumerMob;
                                    //mydata = {
                                    //    label: item[i].PNAME,
                                    //    Pcode: item[i].PCODE,
                                    //    Add: item[i].Add1,
                                    ...
                                    //    transaction.executeSql('INSERT INTO party_master( PCODE,PNAME,Add1,ConsumerMob)VALUES (?,?,?,?)',
                                    //        [ item[i].PCODE,item[i].PNAME,item[i].Add1, item[i].ConsumerMob], nullHandler, errorHandler);
                                    //});
                                    //return mydata;
                                }
                            }
...
function areaall() {
    ...
    function sycronize_item_master() {
        ...
        db.transaction(function (tx) {
            ...
            tx.executeSql('CREATE TABLE IF NOT EXISTS party_master(PCODE  nvarchar(7) , PNAME  TEXT ALLOW NULL, Add1  TEXT ALLOW NULL ,ConsumerMob TEXT ALLOW NULL)', [], nullHandler, errorHandler);
    ...
    function ListDBValues() {
        ...
        db.transaction(function (transaction) {
            ...
            transaction.executeSql('SELECT * FROM party_master ', [],
                 ...
                 // var row = result.rows.item;
                  ...
                  if (result != null && result.rows != null) {
                      for (var i = 0; i < result.rows.length; i++) {
                          ...
                          //    Pcode: row.PCODE,
                          //    Add: row.Add1,
                          //    MobNo: row.ConsumerMob
                          //};
                           
                              
//                           $('#txtOrderParty')._renderItem = function (ul, row) {
//                                           return $('<li></li>')
//                                               .append("<a style='padding-left:5px;padding-right:5px;padding-top:8px;line-height:6vh'><label style='width:90%;white-space:initial;' > " + row.PNAME + "</label>")
//                                               .append("<p style='padding-left:5px;padding-right:5px;padding-top:8px;line-height:6vh;margin:0px'>" + row.Add1 + "(" + row.ConsumerMob + ")</p</a>")
//                                               .append("<hr /></a>")
//                                               .appendTo(ul);
//                                       };
...
//return ;
    ...
    function AddValueToDB() {
        ...
        //table
        //var Pcode = item[i].PCODE;
        //var label = item[i].PNAME;
        //var Pname = item[i].Add1;
        //var mobno = item[i].ConsumerMob;
        db.transaction(function (transaction) {
            transaction.executeSql('INSERT INTO party_master( PCODE,PNAME,Add1,ConsumerMob)VALUES (?,?,?,?)',
                                 [Pcode, label, Pname, mobno], nullHandler, errorHandler);
    ...
    //change comment search  party
   
    //change for geting party//
    $(function () {
        WebSerUrl = localStorage.getItem("APIURL");
        try {
            $('#txtOrderParty').autocomplete({
